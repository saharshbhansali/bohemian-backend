...
def calculate_traditional_votes(election_id: int, db: Session):
    candidate_votes = {candidate.id: 0 for candidate in db.query(Candidate).filter(Candidate.election_id == election_id).all()}
    votes = db.query(Vote).join(Candidate).filter(Candidate.election_id == election_id).all()
    for vote in votes:
