...
# @patch("application.app.datetime.datetime.now", spec=datetime.datetime)
...
def test_get_election_results(client, election_data):
    ...
    future_time = datetime.datetime.now(datetime.UTC) + datetime.timedelta(days=2)
    with patch("application.app.datetime.datetime") as mock_app_datetime:
        mock_app_datetime.now.return_value = future_time
